...
case "$FAULT" in
  latency)
    DELAY="${PARAM:-200ms}"
    echo "[inject_fault] Adding ${DELAY} network latency to $CONTAINER via tc (using docker bypass)..."
    # Use a privileged container to access the Docker Moby VM process namespace
    ...
    docker run --rm --privileged --pid=host alpine:3.20 sh -c \
      "apk add --no-cache iproute2 && (nsenter -t $PID -n tc qdisc add dev eth0 root netem delay ${DELAY_MS}ms 2>/dev/null || nsenter -t $PID -n tc qdisc change dev eth0 root netem delay ${DELAY_MS}ms)"
  ...
  clear-latency)
    ...
    PID=$(docker inspect --format '{{.State.Pid}}' "$CONTAINER")
    docker run --rm --privileged --pid=host alpine:3.20 sh -c \
      "apk add --no-cache iproute2 && nsenter -t $PID -n tc qdisc del dev eth0 root 2>/dev/null || true"
