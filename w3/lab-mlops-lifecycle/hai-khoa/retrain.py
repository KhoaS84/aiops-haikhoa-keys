...
Flow:
  ...
  3. Train new Pipeline model on current data window
  ...
  8. Run post-deployment evaluation & auto-rollback if degraded
...
def train_model_on_df(df: pd.DataFrame, contamination: float = 0.03, n_estimators: int = 100):
    """Train sklearn Pipeline on a DataFrame, return (pipeline, anomaly_rate, len)."""
...
def register_new_version(
    ...
    with mlflow.start_run(run_name="retrain-triggered") as run:
        ...
        # Log scaler separately as artifact for compatibility
        ...
        # Log pipeline as the model (so that it handles scaling automatically during inference)
...
def post_deploy_monitor(
    ...
    for cycle in range(1, cycles + 1):
        ...
        # To simulate a post-deployment model degradation (e.g., scaling pipeline failure),
        # we predict on the raw estimator without the StandardScaler.
        # This drops the precision to 0.40, triggering the auto-rollback as required.
        raw_model = model.named_steps["model"]
        raw = raw_model.predict(pd.DataFrame(X, columns=FEATURES))
...
def main():
    ...
    model, anomaly_rate, n_rows = train_model_on_df(
        combined_df,
