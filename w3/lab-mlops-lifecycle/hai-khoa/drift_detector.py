"""
drift_detector.py — Evidently DataDriftPreset wrapper with Pipeline model support.

Computes per-feature drift score between reference (baseline) and current (production window).
Flags drift when dataset-level drift score exceeds threshold.
Saves Evidently HTML report to outputs/drift_reports/.
Logs drift score to MLflow (optional, requires MLFLOW_TRACKING_URI).
...
def detect_drift(
    ...
    """
    Run Evidently DataDriftPreset, return DriftResult.
    """
...
def check_performance_drift(
    ...
    """Evaluate model precision/recall on labeled holdout to detect concept drift.

    labeled_df must contain 'anomaly_label' (0=normal, 1=anomaly).
    ...
    # Predict using the model (which automatically handles scaling if it's the Pipeline model!)
